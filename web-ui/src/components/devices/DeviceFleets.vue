<template>
  ...
  >
    ...
    <q-separator />
    <q-card-section class="m-0 flex justify-center items-center ">
      <div class="row">
                ...
                <q-item clickable class="pt-2 pb-2 pl-3 pr-3 hoverable" :class="{ selected: (selectedFleet || {}).id === fleet.id }" @click.native="clicked(fleet)">
                  <template>
                    ...
                    <q-item-section>
                      <q-item-label>
                        ...
                        <div v-if="fleet.deviceCount === 0" class="q-item-tile sublabel opacity-40">No device</div>
                  ...
                  <q-item-section avatar>
                    ...
                    <feature-teaser class="inline-block" feature="manage-fleet-update">
                      <q-btn v-if="fleet.deviceCount > 0 && (!fleetDeleteInProgress || fleetDeleteInProgress.id !== fleet.id)" icon="publish" flat @click.stop="createUpdate(fleet)" color="secondary">
              ...
              <drop @drop="deviceAdded(fleet, ...arguments)" @dragover="draggedOver(...arguments)" @dragleave="dragLeave(...arguments)" :key="index">
                <q-item link @click.native="clicked(fleet)" class="p-1 hoverable dash-list-item">
                  ...
                  <q-item-label>
                    ...
                    <div v-if="fleet.deviceCount > 0" class="q-item-tile sublabel opacity-40">
                      {{ fleet.deviceCount }} {{ fleet.deviceCount === 1 ? 'device' : 'devices' }}
                      <!-- <div
...
import CreateFleetDialog from '../fleets/CreateFleetDialog';
