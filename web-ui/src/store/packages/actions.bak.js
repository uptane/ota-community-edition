...
const getSupportData = (context, pkg) => {
  const supportData =
    (context.rootState.ui.supportData.pkgs || []).find((a) => {
      return a.release_type === pkg.buildType || (a.feed && JSON.stringify(pkg).match(new RegExp(a.feed.replaceAll('*', ''), 'i')));
    }) || {};
  return supportData;
};
const _preparePackages = (context, data, delegationInfo) => {
  ...
  const parsedPackages = _.keyBy(
    _.map(packages, (pkg, filepath) => {
      ...
      const supportData = getSupportData(context, pkg);
      return { ...pkg, delegationInfo, supportData, filepath, ...pkg.hashes };
    }),
    'filepathAndHash',
  );
...
export function fetchPackages(context) {
  const { commit, state, dispatch } = context;
  return new Promise((resolve, reject) => {
    commit('ui/setLoadingPackages', true, { root: true });
...
export function saveSelectedDelegations({ dispatch, commit, state }, selectedDelegations) {
  dispatch('ui/setUserOption', { selectedDelegationSources: { ...selectedDelegations } }, { root: true });
}
...
export function refreshDelegations({ dispatch, commit, state }, { delegation }) {
  return new Promise((resolve, reject) => {
    ...
    promise
      .then(() => {
        dispatch('packages/fetchPackages', {}, { root: true }).finally(() => {
          resolve();
        });
      })
      .catch(reject);
...
// /**
//  * Get static delta for specified packages
//  * @param {Object} context
//  * @param {Object} payload
//  */
// export function getStaticDeltas (context, payload) {
//   const { commit } = context;
//   const { packages } = payload;
//   return new Promise((resolve, reject) => {
//     // If no packages are specified, reject the promise
//     if (!packages || packages.length === 0) {
//       reject(new Error('No packages specified'));
//     }
//     const hashes = packages.map((p) => p.hash);

//     // TODO: Remove this once the API is ready
//     // Get all packageHashes from packagesByHash getter
//     const packageHashes = Object.keys(context.getters['packagesByHash']);
//     // If packages are specified, get the static deltas
//     // TODO: Remove this once the API is ready
//     // get hashes beginning with letters
//     const lettersStaticDeltas = packageHashes.filter((h) => h.match(/^[a-zA-Z]/));
//     // get hashes beginning with numbers and filter out the hashes that is already present in storedStaticDeltas
//     const storedStaticDeltas = context.rootState.packages.deltas;
//     const numbersStaticDeltas = hashes.filter((h) => h.match(/^[0-9]/)).filter((h) => !Object.keys(storedStaticDeltas).includes(h));

//     // Prepare a mock data of static deltas object for the specified packages that has hashes beginning with numbers and use a random hash from the packages that has hashes beginning with letters as the to hash. Also use a random size between 10KiB to 10000kiB for the delta file. Use random hashes from the packages that has hashes beginning with numbers as the keys and array of deltas as the value.

//     const staticDeltas = {};
//     numbersStaticDeltas.forEach((h) => {
//       staticDeltas[h] = [];
//       // only add between 1 and 4 deltas for each hash
//       const deltaCount = Math.floor(Math.random() * 4) + 1;
//       for (let i = 0; i < deltaCount; i++) {
//         // get a random hash from the packages that has hashes beginning with letters
//         const toHash = lettersStaticDeltas[Math.floor(Math.random() * lettersStaticDeltas.length)];
//         const fromHash = lettersStaticDeltas[Math.floor(Math.random() * lettersStaticDeltas.length)];
//         // get a random size between 1000KiB to 10000000kiB
//         const size = Math.floor(Math.random() * 10000000 * 1024) + 1000 * 1024;
//         staticDeltas[h].push({
//           to: toHash,
//           from: fromHash,
//           size,
...
export function getStaticDeltas(context) {
  ...
  return new Promise((resolve, reject) => {
    // TODO: Remove this once the API is ready
    // Get all packageHashes from packagesByHash getter
    const packageHashes = Object.values(context.getters['packagesByHash'])
      .filter((p) => p.isOStree)
      .map((p) => p.hash);
    // If packages are specified, get the static deltas
    // TODO: Remove this once the API is ready
    ...
    // get hashes beginning with numbers and filter out the hashes that is already present in storedStaticDeltas
    const storedStaticDeltas = context.rootState.packages.deltas;
    const numbersStaticDeltas = packageHashes.filter((h) => h.match(/^[0-9]/)).filter((h) => !Object.keys(storedStaticDeltas).includes(h));

    // Prepare a mock data of static deltas array for the packages that has hashes beginning with numbers and use a random hash from the packages that has hashes beginning with letters as the to hash. Also use a random size between 10KiB to 1000000kiB for the delta file.

    const staticDeltasArray = [];
    numbersStaticDeltas.forEach((h) => {
      // only add between 0 and 3 deltas for each hash
      const deltaCount = Math.floor(Math.random() * 3) + 0;
      for (let i = 0; i < deltaCount; i++) {
        // get a random hash from the packages that has hashes beginning with letters
        const toHash = lettersStaticDeltas[Math.floor(Math.random() * lettersStaticDeltas.length)];
        const fromHash = lettersStaticDeltas[Math.floor(Math.random() * lettersStaticDeltas.length)];
        // get a random size between 1000KiB to 10000000kiB
        const size = Math.floor(Math.random() * 100000 * 1024) + 1000 * 1024;
        staticDeltasArray.push({
          to: toHash,
          from: fromHash,
          size,
        });
    ...
    staticDeltasArray.push({
      to: '7ae342cef63901d4a3bcdb584604b55cd147dee34cf50beaa13b119b5e74a53d',
      from: 'c70e311ab4db338bf9e853b34bf25dea542016c586d3cc58467df39d14d8ed74',
      size: 29329,
    });

    /*
     The following code is to convert staticDeltasArray to the the format expected by the store, which is an object with the following format:
          {
            'package-hash-1': {
              from: [
                {
          ...
          */
    const staticDeltas = {};
    staticDeltasArray.forEach((delta) => {
    ...
    // // Create an object from the staticDeltasArray in the form {hash:{from[], to:[]}}, mapping the hashes in packageHashes to the array of deltas. For each hash, get the array of deltas that has the hash as the `from` hash and use the array of deltas as the value and the from as the key. Also get the array of deltas that has the hash as the `to` hash and use the array of deltas as the value and the to as the key. Use the hash as the key for the object. Use for loops to do this.
    // const staticDeltas = {};
    // for (let i = 0; i < packageHashes.length; i++) {
    //   const hash = packageHashes[i];
    //   staticDeltas[hash] = {
    //     from: staticDeltasArray.filter((d) => d.to === hash),
    //     to: staticDeltasArray.filter((d) => d.from === hash),
    //   };
    // }
