...
class TufRepoTargetItemsProvider()(implicit val db: Database, val ec: ExecutionContext)
    extends TargetsItemsProvider[Json]
...
class TufRepoSignedRoleProvider(implicit val db: Database, val ec: ExecutionContext)
