...
class CliKeyStorageSpec extends CliSpec with KeyTypeSpecSupport with TryValues {
  ...
  keyTypeTest("writes keys with limited permissions ") { keyType =>
    ...
    val perms =
      Files.getPosixFilePermissions(tempDir.resolve("keys").resolve(keyName.value + ".sec"))
  ...
  test("import RSA public key should work") {
    val rsaPublicKeyPem =
      Paths.get(this.getClass.getResource("/pem-files/valid-public-RSA.pem").toURI)
    val keyName = KeyName("test-rsa-key")

    subject.readPublicKey(keyName).failure.exception.getMessage should include(
  ...
  test("import Ed25519 public key should work") {
    val ed25519PublicKeyPem =
      Paths.get(this.getClass.getResource("/pem-files/valid-public-Ed25519.pem").toURI)
  ...
  test("import EcPrime256 public key should work") {
    val ecPrime256PublicKeyPem =
      Paths.get(this.getClass.getResource("/pem-files/valid-public-EcPrime256.pem").toURI)
  ...
  test("import invalid public key should fail") {
    val rsaPublicKeyPem =
      Paths.get(this.getClass.getResource("/pem-files/invalid-public-RSA.pem").toURI)
    ...
    subject
      .importPublicKey(rsaPublicKeyPem, List(keyName))
      .failure
      .exception
      .getMessage should include(s"Cannot parse public key from $rsaPublicKeyPem")
  }
