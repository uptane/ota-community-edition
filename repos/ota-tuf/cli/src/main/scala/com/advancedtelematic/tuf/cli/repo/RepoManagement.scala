...
object RepoManagement {
  ...
  def initialize(repoServerType: TufServerType,
                 repoPath: Path,
                 initFilePath: Path,
                 repoUri: Option[URI] = None)(implicit ec: ExecutionContext): Try[TufRepo[_]] =
  ...
  def `export`(repo: TufRepo[_], targetKey: KeyName, exportPath: Path): Try[Unit] = {
    ...
    def copyKeyPair(pubKey: TufKey, privKey: TufPrivateKey, dest: ZipOutputStream): Try[Unit] =
      Try {
    ...
    def ensureSourceZipExists(repoPath: Path): Try[ZipFile] = {
      ...
      Try(new ZipFile(zipPath.toFile)).recoverWith { case _: FileNotFoundException =>
    ...
    Try(new ZipOutputStream(new FileOutputStream(exportPath.toFile))).flatMap { zipExportStream =>
      ensureSourceZipExists(repo.repoPath).flatMap { sourceZip =>
        val t = for {
          ...
          _ <- copyRole[RootRole](repo, zipExportStream).recover { case ex =>
...
protected object ZipRepoInitialization {
  ...
  def init(repoServerType: TufServerType,
           repoPath: Path,
           zipTargetKeyName: KeyName,
           initFilePath: Path,
           repoUri: Option[URI])(implicit ec: ExecutionContext): Try[TufRepo[_]] = {
    ...
    def buildOauth(treehubConfig: TreehubConfig): Try[Option[OAuthConfig]] =
      if (treehubConfig.no_auth) Success(None)
        ...
        Either
          .fromOption(treehubConfig.oauth2, TreehubConfigError("auth required with no_auth: false"))
          .toTry
          .map(_.some)
    ...
    def readRepoUri(src: ZipFile): Try[URI] = {
      ...
      for {
        entry <- Try(src.getEntry(filename))
          .filter(_ != null)
          .orElse(Failure(MissingCredentialsZipFile(filename)))
    ...
    for {
      ...
      _ <- writeRoot(src).recover { case ex =>
        _log.warn(
          s"Could not read/write root.json from credentials zip file: ${ex.getMessage}. Continuing."
        )
      }
      _ <- writeTargetKeys(src).recover { case ex =>
        _log.warn(
          s"Could not read/write target keys from credentials zip file: ${ex.getMessage}. Continuing."
        )
      }
      _ = Try(src.close())
    } yield tufRepo
