...
class SignedRoleMigration(old_director_schema: String = "director")(
  ...
  private case class Row(role: RoleType,
                         version: Int,
                         deviceId: DeviceId,
                         content: String,
                         createdAt: Instant,
  ...
  implicit val getRoleTypeResult
    : slick.jdbc.GetResult[com.advancedtelematic.libtuf.data.TufDataType.RoleType.Value] =
    GetResult(r => RoleType.withName(r.nextString()))
  ...
  implicit val getDeviceIdResult
    : slick.jdbc.GetResult[com.advancedtelematic.libats.messaging_datatype.DataType.DeviceId] =
    GetResult(r => DeviceId(UUID.fromString(r.nextString())))
  ...
  implicit val getInstantResult: slick.jdbc.GetResult[java.time.Instant] =
  ...
  private def exists(schema: String, table: String): Future[Boolean] = {
    val sql =
      sql"SELECT EXISTS(SELECT * FROM information_schema.tables WHERE table_schema = $schema AND table_name = $table)"
        .as[Boolean]
  ...
  private def fileCache: Source[Row, NotUsed] = {
    val sql =
      sql""" select role,version,device,file_entity,created_at,updated_at,expires from #$old_director_schema.file_cache"""
