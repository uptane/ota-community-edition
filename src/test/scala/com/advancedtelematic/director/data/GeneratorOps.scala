...
object GeneratorOps {
  ...
  implicit class GenAtMost[T](gen: Gen[T]) {
    ...
    final def listBetween(lower: Int, upper: Int): Gen[List[T]] =
      Gen.choose(lower, upper).flatMap(Gen.containerOfN[List, T](_, gen))
  ...
  def GenRefinedStringByChar[P](gen: Gen[Char])(implicit ev: Validate[String, P]) = GenStringByChar(
    gen
  ).refine
