...
async function checkAllHealth() {
  ...
  for (const service of services) {
    ...
    try {
      ...
      row.textContent = `${service.name}: ${e.message}`;
    }
  }
}
...
async function refreshDevices() {
  ...
  try {
    ...
    addResult(el.deviceList, "error", `Device refresh failed: ${e.message}`);
  }
}
...
async function createGroup() {
  ...
  try {
    ...
    el.deviceNotice.textContent = `Create group failed: ${e.message}`;
  }
}
...
async function refreshGroups() {
  ...
  try {
    ...
    addResult(el.groupList, "error", `Group refresh failed: ${e.message}`);
  }
}
...
async function addDeviceToGroup() {
  ...
  try {
    ...
    el.deviceNotice.textContent = `Add to group failed: ${e.message}`;
  }
}
...
async function removeDeviceFromGroup() {
  ...
  try {
    ...
    el.deviceNotice.textContent = `Remove from group failed: ${e.message}`;
  }
}
...
async function runPreflightChecks() {
  ...
  for (const s of services) {
    try {
      ...
      appendOut(el.preflightOutput, `❌ ${s.name} health failed: ${e.message}`);
    }
  }
  ...
  try {
    ...
    appendOut(el.preflightOutput, `❌ Device check failed: ${e.message}`);
  }
  ...
  try {
    ...
    appendOut(el.preflightOutput, `❌ Group check failed: ${e.message}`);
  }
  ...
  try {
    ...
    appendOut(el.preflightOutput, `⚠️ Membership check failed: ${e.message}`);
  }
...
async function refreshCampaignMonitor() {
  ...
  try {
    ...
    addResult(el.campaignSummary, "error", `Campaign fetch failed: ${e.message}`);
  }
}
...
async function cancelCampaign() {
  ...
  try {
    ...
    el.campaignSummary.innerHTML = "";
    addResult(el.campaignSummary, "error", `Cancel failed: ${e.message}`);
  }
}
...
async function sendManualRequest() {
  ...
  try {
    ...
    el.response.textContent = `Request error: ${error.message}`;
  }
}
